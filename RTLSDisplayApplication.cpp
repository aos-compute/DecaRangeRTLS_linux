    ...
    : QApplication( argc, argv )
{
    ...
    QObject::connect( _client,
                      SIGNAL( anchPos( quint64, double, double, double, bool, bool ) ),
                      graphicsWidget(),
                      SLOT( anchPos( quint64, double, double, double, bool, bool ) ) );
    QObject::connect( _client,
                      SIGNAL( tagPos( quint64, double, double, double ) ),
                      graphicsWidget(),
                      SLOT( tagPos( quint64, double, double, double ) ) );
    QObject::connect( _client,
                      SIGNAL( tagPos( quint64, double, double, double ) ),
                      _tagsPositionProcessor,
                      SLOT( tagPos( quint64, double, double, double ) ) );
    QObject::connect( _client,
                      SIGNAL( tagStats( quint64, double, double, double, double ) ),
                      graphicsWidget(),
                      SLOT( tagStats( quint64, double, double, double, double ) ) );
    QObject::connect( _client,
                      SIGNAL( tagRange( quint64, quint64, double ) ),
                      graphicsWidget(),
                      SLOT( tagRange( quint64, quint64, double ) ) );
    QObject::connect( _client,
    ...
    QObject::connect( _client,
                      SIGNAL( centerOnAnchors( void ) ),
                      graphicsWidget(),
                      SLOT( centerOnAnchors( void ) ) );

    QObject::connect(
        _serialConnection, SIGNAL( clearTags() ), graphicsWidget(), SLOT( clearTags() ) );

    QObject::connect( _serialConnection,
                      SIGNAL( statusBarMessage( QString ) ),
                      _mainWindow,
                      SLOT( statusBarMessage( QString ) ) );
    QObject::connect( _serialConnection,
                      SIGNAL( connectionStateChanged( SerialConnection::ConnectionState ) ),
                      _client,
                      SLOT( connectionStateChanged( SerialConnection::ConnectionState ) ) );

    QObject::connect( graphicsWidget(),
                      SIGNAL( updateAnchorXYZ( int, int, double ) ),
                      _client,
                      SLOT( updateAnchorXYZ( int, int, double ) ) );
    QObject::connect( graphicsWidget(),
                      SIGNAL( updateTagCorrection( int, int, int ) ),
                      _client,
                      SLOT( updateTagCorrection( int, int, int ) ) );

    QObject::connect( _client,
                      SIGNAL( ancRanges( int, int, int ) ),
                      graphicsWidget(),
                      SLOT( ancRanges( int, int, int ) ) );
...
void RTLSDisplayApplication::connectReady( QObject* receiver,
                                           const char* member,
                                           Qt::ConnectionType type )
{
    QMetaMethod method = receiver->metaObject()->method(
        receiver->metaObject()->indexOfMethod( QMetaObject::normalizedSignature( member ) ) );
    ...
    if ( instance()->_ready && method.isValid() )
        ...
        QObject::connect( instance(),
                          QMetaMethod::fromSignal( &RTLSDisplayApplication::ready ),
                          receiver,
                          method,
                          type );
}
